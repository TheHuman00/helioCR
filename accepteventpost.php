...
try {
    ...
    $mail->isHTML(true);                                
    $mail->Subject = "Ajouté à l'événement - $titre";
    $mail->Body    =  $body8;
	...
	if($att == "false"){
		  ...
		  $query2 .=") VALUES (";
		  $query2 .="'{$userr}', '{$eventt}', '{$date['1']}', '2', '{$p_comm}', '', ''";
		  $query2 .=") ON DUPLICATE KEY UPDATE user ='{$userr}', event ='{$eventt}', date='{$date['1']}', dispo ='2', comm ='{$p_comm}'";
		  $result2 = $db->query($query2);
		  if($result2){
			foreach($disponibilites as $disponibilite){
				if($disponibilite['user'] == $userr){
					if(str_contains($disponibilite['event'], $calendrier['date'])){
					if(str_contains($disponibilite['event'], $calendrier['title'])){
					  $session->msg("s", "Accepté donné avec succes.");
					}else{
					  if($disponibilite['dispo'] == "0"){
						$session->msg("s", "Accepté donné avec succes.");
					  }else{
						if($calendrier['doubleprev'] == "non" && $disponibilite['dispo'] == "0"){
							$sql = $database->db_prepare("DELETE FROM disponibilite WHERE id= ? LIMIT 1");
							$sql->bind_param('s', $disponibilite['id']);
							$result = $sql->execute();
						  if($result){
							$session->msg('s',"Double prev impossible suppression de participation à un");
						  }
						}else{
						  $session->msg("s", "Accepté avec succes.");
						}
					  }
					}
				  }else{
			...
			$session->msg("d","Erreur lors de l'acceptation. $eventt");
			redirect("eventadmin?nom=$eventt");
		  }
	};
	if($att == "true"){
			  ...
			  $query2 .="user,event,date,dispo,comm,rolecp,rolechau";
			  $query2 .=") VALUES (";
			  $query2 .="'{$userr}', '{$eventt}', '{$date['1']}', '1', '{$p_comm}', '', ''";
			  $query2 .=") ON DUPLICATE KEY UPDATE user ='{$userr}', event ='{$eventt}', date='{$date['1']}', dispo ='1', comm ='{$p_comm}'";
			  $result2 = $db->query($query2);
			  if($result2){
				foreach($disponibilites as $disponibilite){
					if($disponibilite['user'] == $userr){
				...
				$session->msg("d","Erreur lors de la mise en attente. $eventt");
				redirect("eventadmin?nom=$eventt");
			  }
	}
