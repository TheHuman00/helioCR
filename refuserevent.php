...
<table border="0" cellpadding="0" cellspacing="0" align="center"
	...
	<tr>
		<td align="center" valign="top" style="border-collapse: collapse; border-spacing: 0; margin: 0; padding: 0; padding-left: 6.25%; padding-right: 6.25%; width: 87.5%; font-size: 17px; font-weight: 400; line-height: 160%;
			...
			color: #000000;
			font-family: sans-serif;" class="paragraph">
				Merci pour ton dévouement ! Nous ferons notre possible pour vous faites participer à un autre préventif.
		</td>
	</tr>
</table>
...
try {
	...
	$mail->isHTML(true);
	$mail->Subject = "Refus à l'événement - $titre";
	$mail->Body    =  $body8;
	$mail->AltBody = 'Charger le mail HTML';

	$mail->send();
	$resultatdemande = find_disponibilite_by_title($eventt, $userr);
	$update_dispo = update_dispo('0', (int)$resultatdemande['id']);
	if ($update_dispo) {
		$sql = "UPDATE disponibilite SET rolecp='non', rolechau='non' WHERE id='{$resultatdemande['id']}'";
		$result = $db->query($sql);
		if ($result) {
			$session->msg("s", "Email de refus envoyé avec succes.");
			redirect("eventadmin?nom=$eventt");
		} else {
			$session->msg("d", "Erreur lors du refus.");
			redirect("eventadmin?nom=$eventt");
		}
	} else {
		$session->msg("d", "Erreur lors du refus.");
